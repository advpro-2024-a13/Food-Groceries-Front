...
const KeranjangBelanjaPage: React.FC = () => {
  ...
  useEffect(() => {
    if (ownerId !== null) {
      fetch(`http://localhost:8080/keranjangbelanja/getKeranjangBelanjaById?ownerId=${ownerId}`)
  ...
  const handleCreate = () => {
    if (ownerId !== null) {
      fetch(`http://localhost:8080/keranjangbelanja/createNewKeranjangBelanja?ownerId=${ownerId}`, {
        method: 'POST',
      })
        .then((response) => response.json())
        .then((data) => setKeranjangBelanja(data))
  ...
  const handleAddProduct = () => {
    if (ownerId !== null) {
      fetch(`http://localhost:8080/keranjangbelanja/addProductToKeranjangBelanja`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdateProductQuantity = (productId: string, newQuantity: number) => {
    if (ownerId !== null) {
      fetch(`http://localhost:8080/keranjangbelanja/updateProductQuantity`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ ownerId, productId, quantity: newQuantity }),
      })
        .then((response) => response.json())
        .then((data) => setKeranjangBelanja(data))
    }
  ...
  const handleClear = () => {
    if (ownerId !== null) {
      fetch(`http://localhost:8080/keranjangbelanja/clearKeranjangBelanja?ownerId=${ownerId}`, {
        method: 'DELETE',
      })
        .then((response) => response.json())
        .then((data) => setKeranjangBelanja(data))
  ...
  const handleRemoveProduct = () => {
    if (ownerId !== null) {
      fetch(`http://localhost:8080/keranjangbelanja/removeProductFromKeranjangBelanja`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <main className="py-28 flex justify-center items-center">
      <div>
        <Card style={{ marginBottom: '20px', width: '600px' }}>
          ...
          <CardContent>
            {/* Render your keranjangBelanja data here */}
            {keranjangBelanja && (
